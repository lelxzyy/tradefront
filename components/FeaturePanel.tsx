        ...
        <section className="panel">
          ...
          {p.candles.length ? (
            ...
            <Empty
              ...
              text="Tidak ada API key aktif, key sedang cooldown, atau kredit Twelve Data telah habis. Periksa menu API key pool."
            />
          )}
        </section>
      </Page>
...
function ApiKeyManagement(){
  ...
  return <Page title="Twelve Data API pool" sub="Rotasi otomatis ketika kredit salah satu API key habis">
    ...
    <section className="panel journal-table user-table"><div className="panel-head"><b>API credits</b><button className="pool-refresh" onClick={()=>void load(true)}><ArrowClockwise/> Refresh credits</button></div><table><thead><tr><th>PRIORITY</th><th>LABEL / KEY</th><th>CREDITS</th><th>ROTATION</th><th>ACTIONS</th></tr></thead><tbody>{keys.length?keys.map(x=><tr key={x.id}><td>#{x.priority}</td><td><b>{x.label}</b><small>{x.maskedKey} · {!x.active?"DISABLED":x.disabledUntil&&new Date(x.disabledUntil)>new Date()?"COOLDOWN":"ACTIVE"}</small></td><td className={(x.remaining??1)===0?"red":"green"}>{x.remaining!==undefined?`${x.remaining} / ${x.limit}`:"—"}</td><td>{x.disabledUntil&&new Date(x.disabledUntil)>new Date()?`Retry ${new Date(x.disabledUntil).toLocaleString("id-ID",{timeZone:"Asia/Jakarta"})}`:"READY"}</td><td><div className="user-actions"><button onClick={()=>void call("PATCH",{id:x.id,active:!x.active})}>{x.active?"Disable":"Enable"}</button>{x.disabledUntil&&new Date(x.disabledUntil)>new Date()&&<button onClick={()=>void call("PATCH",{id:x.id,retry:true})}>Retry now</button>}<button onClick={()=>{const value=window.prompt("API key baru");if(value)void call("PATCH",{id:x.id,key:value,retry:true})}}>Change</button><button onClick={()=>{const value=window.prompt("Priority",String(x.priority));if(value)void call("PATCH",{id:x.id,priority:Number(value)})}}>Priority</button><button className="delete" onClick={()=>{if(confirm(`Hapus ${x.label}?`))void call("DELETE",undefined,`?id=${x.id}`)}}>Delete</button></div></td></tr>):<tr><td colSpan={5}>Belum ada API key.</td></tr>}</tbody></table></section>
  </Page>
}
