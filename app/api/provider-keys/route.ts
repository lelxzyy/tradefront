...
export async function GET() {
  ...
  const data=await Promise.all(rows.map(async ({_id,encryptedKey,...x})=>{
    ...
    try { const response=await fetch("https://api.twelvedata.com/api_usage",{headers:{Authorization:`apikey ${plain}`},cache:"no-store"}); const u=await response.json(); if(response.ok&&u.plan_daily_limit){usage={used:Number(u.daily_usage),limit:Number(u.plan_daily_limit),remaining:Math.max(0,Number(u.plan_daily_limit)-Number(u.daily_usage)),usageCheckedAt:new Date()};await collection.updateOne({_id},{$set:usage});} } catch {}
